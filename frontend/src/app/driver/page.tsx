...
export default function DriverDashboardPage() {
    ...
    // Direct cancel modal state (within 1 hour)
    const [directCancelTrip, setDirectCancelTrip] = useState<Trip | null>(null);
    const [directCancelReason, setDirectCancelReason] = useState('');
    const [directCancelCustom, setDirectCancelCustom] = useState('');

    const CANCEL_REASONS_AR = [
        'تغيّرت خططي',
        'مشكلة في السيارة',
        'ظروف طارئة',
        'لا يوجد ركاب',
    ...
    const handleDirectCancel = async () => {
        if (!directCancelTrip) return;
        const finalReason = directCancelReason === 'أخرى'
            ? directCancelCustom.trim() || 'أخرى'
            : directCancelReason;
        if (!finalReason) {
            toast.error('يرجى اختيار سبب الإلغاء');
            return;
        }
        setActionLoading(directCancelTrip.id);
        try {
            await api.cancelTrip(directCancelTrip.id, finalReason);
            toast.success('تم إلغاء الرحلة بنجاح. سيتم إخطار الركاب.');
            setDirectCancelTrip(null);
            setDirectCancelReason('');
            setDirectCancelCustom('');
            load();
        } catch (err: any) {
            toast.error(err.message || 'فشل إلغاء الرحلة');
        } finally {
    ...
    return (
        <ProtectedRoute allowedRoles={['DRIVER', 'ADMIN']}>
                                                ...
                                                <div className="flex flex-col gap-3 border-t border-zinc-200 pt-4 sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        ...
                                                        {canCancelTrip(trip) && isWithinOneHour(trip) && (
                                                            <button
                                                                onClick={() => setDirectCancelTrip(trip)}
            ...
            {/* DIRECT CANCEL MODAL (within 1 hour) — Arabic UI */}
            {directCancelTrip && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setDirectCancelTrip(null)}>
                    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-zinc-900" onClick={(e) => e.stopPropagation()}>
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-lg font-bold text-zinc-900 dark:text-white">
                                إلغاء الرحلة ❌
                            </h3>
                            <button onClick={() => { setDirectCancelTrip(null); setDirectCancelReason(''); setDirectCancelCustom(''); }} className="rounded-lg p-1 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800">
                                <X className="h-5 w-5" />
                            </button>
                        </div>

                        <p className="mb-2 text-sm text-zinc-500 dark:text-zinc-400" dir="rtl">
                            {directCancelTrip.fromCity} ← {directCancelTrip.toCity} — {new Date(directCancelTrip.departureTime).toLocaleDateString('ar-EG')}
                        </p>

                        <p className="mb-4 text-sm font-semibold text-zinc-800 dark:text-zinc-200" dir="rtl">
                            لماذا تريد إلغاء هذه الرحلة؟
                        </p>

                        <div className="mb-4 space-y-2" dir="rtl">
                            {CANCEL_REASONS_AR.map((reason) => (
                                <label
                                    key={reason}
                                ...
                                >
                                    <input
                                        ...
                                        name="cancelReason"
                                        value={reason}
                                        checked={directCancelReason === reason}
                                        onChange={() => setDirectCancelReason(reason)}
                        ...
                        {directCancelReason === 'أخرى' && (
                            <div className="mb-4" dir="rtl">
                                <textarea
                                    value={directCancelCustom}
                                    onChange={(e) => setDirectCancelCustom(e.target.value)}
                                    rows={2}
                                    placeholder="اكتب سبب الإلغاء هنا..."
                                    className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm text-right focus:border-red-500 focus:ring-2 focus:ring-red-500/20 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
                                />
                            </div>
                        )}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => { setDirectCancelTrip(null); setDirectCancelReason(''); setDirectCancelCustom(''); }}
                                className="flex-1 rounded-lg border border-zinc-200 px-4 py-2.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300"
                            >
                                رجوع
                            </button>
                            <button
                                ...
                                disabled={!directCancelReason || (directCancelReason === 'أخرى' && !directCancelCustom.trim()) || actionLoading === directCancelTrip.id}
                                className="flex-1 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-500 disabled:opacity-50"
                            >
                                {actionLoading === directCancelTrip.id ? 'جاري الإلغاء...' : 'تأكيد الإلغاء'}
                            </button>
                        </div>
                    </div>
                </div>
